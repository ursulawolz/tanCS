def makeTableStructure1(host1 , password , username, personalpassword) :
	...
	cur.execute("create table comment(all_of_the_text longtext, account_posting_id smallint, project_id smallint, revision_number smallint, file_name tinytext, line_number tinyint, comment_id int, creation_time datetime);")
	cur.execute("create table file_info( project_id smallint, revision_number smallint, file_name tinytext, file_content blob, comments text, creation_time datetime);")
